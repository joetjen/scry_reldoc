...
- The `relational` + `document` composite kind (impl_spec.md §2/§6) -- a thin delegate, not a fused executor: `Scry.Reldoc.parse/1`/`Scry.Reldoc.Executor.run/3` both call straight through to `Scry.Document.parse/1`/`Scry.Document.Executor.run/3`. `relational` is a degenerate kind (no EP1/EP2 grammar/execution vocabulary of its own).
